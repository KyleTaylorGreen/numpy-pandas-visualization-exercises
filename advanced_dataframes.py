...
if branch == 'PT3':
    ...
    # 1) Use your get_db_url function to help you explore the data from the 
    #    chipotle database.
    ...
    print(chipotle.groupby('item_name').quantity.sum().nlargest(n=3, keep='all'))


    # 4) Which item has produced the most revenue?

    print(chipotle.groupby('item_name').item_price.sum().idxmax())

    # 5) Join the employees and titles DataFrames together.

    url = get_db_url('employees')
    sql = """
    SELECT dept_name, title, hire_date
    FROM employees
    JOIN titles USING(emp_no)
    JOIN dept_emp USING(emp_no)
    JOIN departments USING(dept_no);
    """
    emp_titles_joined = pd.read_sql(sql, url)
    print(emp_titles_joined.head())

    # 6) For each title, find the hire date of the employee that was hired 
    #    most recently with that title.

    print(emp_titles_joined.groupby('title').hire_date.max())

    # 7) Write the code necessary to create a cross tabulation of the number 
    # of titles by department. (Hint: this will involve a combination of SQL 
    # code to pull the necessary data and python/pandas code to perform the manipulations.)

    num_titles_by_dept = pd.crosstab(index=emp_titles_joined.dept_name, columns=emp_titles_joined.title)
    print(num_titles_by_dept)